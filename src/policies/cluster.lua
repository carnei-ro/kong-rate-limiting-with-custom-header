  ...
  cassandra = {
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          local res, err = connector:query([[
            UPDATE ratelimiting_with_custom_header_metrics
               SET value = value + ?
             WHERE identifier = ?
               AND period = ?
               AND period_date = ?
    ...
    find = function(connector, identifier, period, current_timestamp, service_id, route_id)
      ...
      local rows, err = connector:query([[
        SELECT value
          FROM ratelimiting_with_custom_header_metrics
         WHERE identifier = ?
           AND period = ?
           AND period_date = ?
           AND service_id = ?
    ...
    increment = function(connector, limits, identifier, current_timestamp, service_id, route_id, value)
      ...
      for period, period_date in pairs(periods) do
        if limits[period] then
          len = len + 1
          buf[len] = fmt([[
            INSERT INTO "ratelimiting_with_custom_header_metrics" ("identifier", "period", "period_date", "service_id", "route_id", "value")
                 VALUES ('%s', '%s', TO_TIMESTAMP('%s') AT TIME ZONE 'UTC', '%s', '%s', %d)
            ON CONFLICT ("identifier", "period", "period_date", "service_id", "route_id") DO UPDATE
                    SET "value" = "ratelimiting_with_custom_header_metrics"."value" + EXCLUDED."value"
